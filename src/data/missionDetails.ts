// ---------------------------------------------------------------------------
// Détails complets des missions médicales ASFO — source unique pour la page
// détail /archives/:id (récit, points forts, galerie, témoignages, impact).
// Extrait verbatim de l'ancien SingleArchivePage (aucun chiffre modifié).
// ---------------------------------------------------------------------------

export interface MissionSpecialty {
  name: string;
  count: number;
}
...
const missionDetailsSource: MissionDetail[] = [
  // ===== 2024 MISSIONS =====
  {
...
L'organisation exemplaire et l'engagement de toutes les équipes ont fait de cette mission une référence.`,
    ...
    gallery: [
      "/village-ndouloumadji-founebe.webp"
    ],
    testimonials: [
      {
        name: "Ibrahima Sow",
...
Cette mission a marqué un tournant dans notre approche des soins ophtalmologiques en milieu rural.`,
    ...
    gallery: [
      "/village-doumga-ouro-alpha.webp"
    ],
    testimonials: [
      {
        name: "Fatou Diallo",
...
/*
 * Les galeries détaillées ne sont publiées que lorsque leurs fichiers sont
 * réellement disponibles. Les autres missions réutilisent leur visuel
 * principal afin d'éviter toute image cassée sans inventer de contenu.
 */
const missionsWithDocumentedGallery = new Set([
  '2024-guede-village',
  '2024-village-tatqui',
...
export const missionDetails: MissionDetail[] = missionDetailsSource.map((mission) =>
  missionsWithDocumentedGallery.has(mission.id)
    ? mission
    : { ...mission, gallery: [mission.imageUrl] },
);
